...
function reminders_get( $eventid, &$mform ) {
    ...
    if ( ! $reminders = $DB->get_records( 'reminders', array( 'event'=>$eventid, 'sent'=>0 ) ) ) return false;

    if ( isset( $COURSE->id ) AND $COURSE->id ) $crsstr = '&id=' . $COURSE->id;
    else $crsstr = '';
    ...
    foreach( $reminders as $rem ) {
        ...
        if ( $i++ > $CFG->local_reminders ) break;
    }
    return true;
}
...
function reminders_form( &$mform, $check=false, $checktodisable = false, $addheader = true, $aftertypetwoonly = false ) {
    ...
    if( $addheader ) $mform->addElement('header', 'remhdr', get_string('remhdr', 'local_reminders'), '', array("style" => "display:none"));
    if ( $check ) $mform->addElement('checkbox', 'reminders', '', get_string('link','local_reminders'));
...
function reminders_send( $code, $event=null, $user=null, $course=null, $other=null ) {
    ...
    if ( empty( $user ) AND !empty( $USER ) ) $user   = $USER;
    if ( empty( $user ) AND !empty( $event ) AND $event->userid ) $user = $DB->get_record( 'user', array( 'id'=>$event->userid ) );
    if ( empty( $user ) ) return false;
    if ( empty( $user->description ) ) $user->description = '';

    // Get Message we're dealing with
    if ( empty( $message ) AND !empty( $course->lang ) ) $message = $DB->get_record( 'reminder_templates', array( 'code'=>$code, 'lang'=>$course->lang ) );
    if ( empty( $message ) AND !empty( $user->lang  ) )  $message = $DB->get_record( 'reminder_templates', array( 'code'=>$code, 'lang'=>$user->lang ) );
    if ( empty( $message ) AND !empty( $CFG->lang    ) ) $message = $DB->get_record( 'reminder_templates', array( 'code'=>$code, 'lang'=>$CFG->lang ) );
    if ( empty( $message ) )                             $message = $DB->get_record( 'reminder_templates', array( 'code'=>$code ) );
    if ( empty( $message ) ) return false;

    // Deal with an empty course record
    if ( empty( $course ) AND !empty( $COURSE ) ) $course = $COURSE;
    if ( empty( $course ) AND !empty( $event ) AND $event->courseid ) $course = $DB->get_record( 'course', array( 'id'=>$event->courseid ) );
    if ( empty( $course ) ) {
    ...
    if ( empty( $event ) AND !empty( $course ) AND $DB->count_records( 'event', array( 'courseid'=>$course->id ) ) == 1 ) $event = $DB->get_record( 'event', array( 'courseid'=>$course->id ) );
    if ( empty( $event ) AND $DB->count_records( 'event', array( 'userid'=>$user->id ) ) == 1 ) $event = $DB->get_record( 'event', array( 'userid'=>$user->id ) );
    if ( empty( $event ) ) {
        $event = new stdClass;
...
function reminders_get_course_users( $event ) {
    ...
    if ( $connect = $DB->get_record( 'connectmeeting', array( 'eventid'=>$event->id ) ) ) {
        if ( $cm = get_coursemodule_from_instance( 'connectmeeting', $connect->id, $connect->course ) ) {
            if ( $cm->groupmode == 1 AND $cm->groupingid  ) $groupingid = $cm->groupingid;
        }
    } elseif ( $instance = $DB->get_record( 'connectmeeting_recurring', array( 'eventid'=>$event->id, 'record_used' => 0 ) ) ) {
        if ( $connect = $DB->get_record( 'connectmeeting_recurring', array( 'id'=>$instance->connectid ) ) ) {
            if ( $cm = get_coursemodule_from_instance( 'connectmeeting_recurring', $connect->id, $connect->course ) ) {
                if ( $cm->groupmode == 1 AND $instance->groupingid ) $groupingid = $instance->groupingid;
            }
        }
    ...
    } elseif ( isset( $event->courseid ) AND $event->courseid ) $users = get_enrolled_users( context_course::instance($event->courseid ) );
    else return false;
...
function local_user_callback( $link ) {
    ...
    if ( empty( $link[1] ) ) return;
    $field = strip_tags(strtolower( $link[1] ));
    if ( isset( $CURUSER->$field ) ) return $CURUSER->$field;
    if ( isset( $CURUSER->profile->$field ) ) return $CURUSER->profile->$field;
    if ( isset( $CURUSER->profile[$field] ) ) return $CURUSER->profile[$field];
...
function local_other_callback( $link ) {
    ...
    if ( isset( $OTHER->$field ) ) return $OTHER->$field;
